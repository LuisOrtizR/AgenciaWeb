...
import { param, uid }                                        from '../../tipos.js'
...
export const obtenerPorId = async (req: Request, res: Response): Promise<void> => {
  try {
    respuestaExito(res, await servicio.obtenerUsuarioPorId(param(req, 'id')))
  } catch (error) { manejarError(res, error, 404) }
}
...
export const actualizar = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!resultado.success) { respuestaError(res, 'Datos invalidos', 400, resultado.error.flatten().fieldErrors); return }
  try {
    respuestaExito(res, await servicio.actualizarUsuario(param(req, 'id'), resultado.data), 'Usuario actualizado correctamente')
  } catch (error) { manejarError(res, error) }
}

export const activar = async (req: Request, res: Response): Promise<void> => {
  try {
    respuestaExito(res, await servicio.activarUsuario(param(req, 'id')), 'Usuario activado correctamente')
  } catch (error) { manejarError(res, error) }
}

export const desactivar = async (req: Request, res: Response): Promise<void> => {
  try {
    respuestaExito(res, await servicio.desactivarUsuario(param(req, 'id'), uid(req)), 'Usuario desactivado correctamente')
  } catch (error) { manejarError(res, error) }
}
...
export const cambiarRol = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!resultado.success) { respuestaError(res, 'Datos invalidos', 400, resultado.error.flatten().fieldErrors); return }
  try {
    const datos = await servicio.cambiarRol(param(req, 'id'), resultado.data.rol, uid(req))
...
export const eliminar = async (req: Request, res: Response): Promise<void> => {
  try {
    respuestaExito(res, await servicio.eliminarUsuario(param(req, 'id'), uid(req)), 'Usuario eliminado permanentemente')
  } catch (error) { manejarError(res, error) }
}