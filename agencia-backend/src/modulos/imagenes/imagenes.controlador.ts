...
import { SolicitudAutenticada, param }                       from '../../tipos.js'
...
const buscarProyecto = async (id: string, res: Response) => {
  ...
  if (!proyecto) { respuestaError(res, 'Proyecto no encontrado', 404); return null }
  return proyecto
}
...
export const subirImagenProyecto = async (req: SolicitudAutenticada, res: Response): Promise<void> => {
  try {
    const id = param(req, 'id')
    if (!req.file) { respuestaError(res, 'No se recibio ningun archivo. Envia el campo "imagen".', 400); return }
    const proyecto = await buscarProyecto(id, res)
    if (!proyecto) return
    await limpiarImagenAnterior(proyecto.imagenUrl)
    const { url, publicId } = await subirImagen(req.file.buffer, CARPETAS.proyectos, proyecto.slug)
    ...
    respuestaExito(res, { ...proyectoActualizado, publicId }, 'Imagen subida exitosamente', 201)
  } catch (error) {
...
export const eliminarImagenProyecto = async (req: SolicitudAutenticada, res: Response): Promise<void> => {
  try {
    const id      = param(req, 'id')
    const proyecto = await buscarProyecto(id, res)
    if (!proyecto) return
    if (!proyecto.imagenUrl) { respuestaError(res, 'Este proyecto no tiene imagen que eliminar', 409); return }
