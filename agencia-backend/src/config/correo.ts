...
export const notificarNuevoProspecto = async (datos: DatosProspectoCorreo): Promise<void> => {
  ...
  const cuerpo = `
    <p style="color:#475569;font-size:15px;margin:0 0 20px;">Alguien llenó el formulario de contacto:</p>
    ${campo('Nombre', datos.nombre)}
    ${campo('Correo', `<a href="mailto:${datos.correo}" style="color:#7c3aed;">${datos.correo}</a>`)}
    ${datos.telefono ? campo('Teléfono', `<a href="tel:${datos.telefono}" style="color:#7c3aed;">${datos.telefono}</a>`) : ''}
    ...
    <div style="background:#eff6ff;border-left:4px solid #7c3aed;border-radius:0 8px 8px 0;padding:14px 18px;margin-top:8px;">
      <p style="margin:0;color:#1e40af;font-size:14px;">💡 <strong>Próximo paso:</strong> Contacta a este prospecto lo antes posible.</p>
    </div>
  `
  await enviar(entorno.EMPRESA_CORREO_SOPORTE, `🎯 Nuevo prospecto: ${datos.nombre} — ${tipo}`, plantillaBase(cuerpo, '🎯 Nuevo Prospecto'))
}
...
export const confirmarRecepcionProspecto = async (datos: DatosProspectoCorreo): Promise<void> => {
  ...
  const cuerpo = `
    ...
    <p style="color:#475569;font-size:15px;line-height:1.7;margin:0 0 20px;">
      Recibimos tu solicitud y nos comunicaremos contigo en las próximas <strong>24 horas hábiles</strong>.
    </p>
  ...
  `
  await enviar(datos.correo, `✅ Recibimos tu solicitud — ${entorno.EMPRESA_NOMBRE}`, plantillaBase(cuerpo, '¡Gracias por contactarnos!'))
}
...
export const notificarNuevoTestimonio = async (datos: DatosTestimonioCorreo): Promise<void> => {
  ...
  const cuerpo = `
    <p style="color:#475569;font-size:15px;margin:0 0 20px;">Un cliente envió un testimonio. Apruébalo desde el panel.</p>
    ...
    <div style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 8px 8px 0;padding:14px 18px;">
      <p style="margin:0;color:#92400e;font-size:14px;">⚡ <strong>Acción requerida:</strong> Panel → Testimonios → Pendientes.</p>
    </div>
  `
  await enviar(entorno.EMPRESA_CORREO_SOPORTE, `⭐ Testimonio pendiente: ${datos.nombreCliente} (${datos.calificacion}/5)`, plantillaBase(cuerpo, '⭐ Nuevo Testimonio Pendiente'))
}
...
export const enviarCotizacion = async (datos: DatosCotizacionCorreo): Promise<void> => {
  ...
  const cuerpo = `
    ...
    <p style="color:#475569;font-size:15px;margin:0 0 24px;">Preparamos tu cotización personalizada:</p>
    ...
    <p style="color:#475569;font-size:14px;margin:0;">Para aceptar esta cotización o resolver dudas, responde este correo.</p>
  `
  await enviar(datos.correoCliente, `📄 Tu cotización de ${entorno.EMPRESA_NOMBRE} — ${formatearPeso(datos.precioTotal)}`, plantillaBase(cuerpo, '📄 Tu Cotización'))
}
...
export const enviarBienvenida = async (datos: DatosBienvenidaCorreo): Promise<void> => {
  const cuerpo = `
    <p style="color:#1e293b;font-size:16px;margin:0 0 12px;">Hola <strong>${datos.nombre}</strong> 👋</p>
    <p style="color:#475569;font-size:15px;line-height:1.7;margin:0 0 20px;">
      Tu cuenta en <strong>${entorno.EMPRESA_NOMBRE}</strong> fue creada exitosamente. Ya puedes iniciar sesión.
    </p>
