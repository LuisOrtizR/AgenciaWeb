...
const tituloPagina = computed(() => {
  return (route.meta.titulo as string | undefined)?.split(' — ')[0] ?? 'Panel'
})
  ...
  <header class="h-16 shrink-0 flex items-center justify-between px-4 sm:px-6 bg-[#0d0d16] border-b border-white/5">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-gray-500 text-xs mt-0.5 hidden sm:block">Nexova Studio Admin</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      ...
      <RouterLink
        :to="{ name: 'inicio' }"
        target="_blank"
        class="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10 transition-all font-medium"
      >
      ...
      <div class="relative">
        <button
          class="flex items-center gap-2.5 pl-1 pr-3 py-1.5 rounded-xl hover:bg-white/5 transition-all group"
        ...
        >
          ...
          <div class="hidden sm:block text-left">
            ...
            <p class="text-[10px] text-gray-500 mt-0.5 capitalize">{{ authStore.usuario?.rol?.toLowerCase() }}</p>
          </div>
          <svg class="w-3.5 h-3.5 text-gray-500 transition-transform hidden sm:block" :class="menuUsuario ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <Transition name="dropdown">
          <div
            v-if="menuUsuario"
            v-click-outside="() => menuUsuario = false"
