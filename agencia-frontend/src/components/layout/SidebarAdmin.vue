  ...
  >
    <div class="flex items-center h-16 px-4 border-b border-white/5 shrink-0">
      <RouterLink :to="{ name: 'admin-dashboard' }" class="flex items-center gap-3 min-w-0">
        <div class="shrink-0 w-8 h-8 rounded-xl bg-linear-to-br from-violet-600 to-indigo-500 flex items-center justify-center shadow-lg shadow-violet-600/25">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-4 px-2 space-y-5">
      <div v-for="seccion in secciones" :key="seccion.titulo">
        <p v-show="abierto" class="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-600 select-none">
          {{ seccion.titulo }}
        </p>
        <div v-show="!abierto" class="h-px bg-white/5 mx-2 mb-2" />
          ...
          <li v-for="enlace in seccion.enlaces" :key="enlace.nombre">
            ...
            >
              <span
                ...
                class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-violet-400 rounded-r-full"
              />
              <svg
                class="shrink-0 transition-colors"
                :class="esActivo(enlace.ruta) ? 'text-violet-400' : 'text-slate-500'"
