...
const analizarConIA = async (prospecto: Prospecto) => {
  ...
  try {
    ...
    const nivelColor: Record<string, string> = { CALIENTE: '🔴', TIBIO: '🟡', FRIO: '🔵' }
    resultadoIA.value = [
      `${nivelColor[a.nivel] ?? '⚪'} PUNTAJE: ${a.puntaje}/100 — ${a.nivel}`,
      '',
      '📋 RESUMEN',
      a.resumen,
      '',
...
<template>
  <div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="bg-[#13151f] border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
      ...
      <select v-model="filtros.estado" class="px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-blanco-suave text-sm outline-none">
        <option v-for="e in ESTADOS" :key="e.valor" :value="e.valor">{{ e.etiqueta }}</option>
      </select>
      <select v-model="filtros.tipoServicio" class="px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-blanco-suave text-sm outline-none">
            ...
            <tr v-for="p in prospectos" :key="p.id" class="hover:bg-white/2 transition-colors">
              ...
              <td class="px-4 py-4">
                <button @click="abrirModalEstado(p)">
                  <AppInsignia :variante="varianteEstado(p.estado)" punto>{{ etiquetaEstado(p.estado) }}</AppInsignia>
                </button>
              </td>
    ...
    <AppModal :abierto="modalEstado" titulo="Actualizar estado del prospecto" tamano="sm" @cerrar="modalEstado = false">
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          ...
          <select v-model="formularioEstado.estado" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 focus:border-violeta/50 text-white text-sm outline-none transition-all">
