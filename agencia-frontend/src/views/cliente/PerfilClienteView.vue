...
const guardarPerfil = async () => {
  ...
  try {
    ...
    uiStore.error('Error', e instanceof Error ? e.message : 'Error al guardar')
  } finally {
...
const cambiarContrasena = async () => {
  ...
  try {
    ...
    uiStore.error('Error', e instanceof Error ? e.message : 'Contraseña actual incorrecta')
  } finally {
    ...
    <template v-else>
      ...
      <div v-if="tabActiva === 'perfil'" class="bg-[#13151f] border border-white/5 rounded-2xl p-6 space-y-5">
        ...
        <div class="space-y-1.5">
          ...
          <input
            ...
            class="w-full px-4 py-3 rounded-xl bg-white/5 border text-white placeholder-slate-500 text-sm outline-none transition-colors"
            :class="erroresPerfil.nombre ? 'border-red-500/50' : 'border-white/10 focus:border-violet-500/50'"
          />
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-slate-300">
            Correo electrónico
            <span class="ml-2 text-xs text-slate-500 font-normal">(no editable)</span>
          </label>
          <input
            :value="authStore.usuario?.correo"
            type="email"
            readonly
            class="w-full px-4 py-3 rounded-xl bg-white/3 border border-white/5 text-slate-500 text-sm outline-none cursor-not-allowed"
          />
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <input
            ...
            class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-violet-500/50 text-white placeholder-slate-500 text-sm outline-none transition-colors"
          />
        </div>

        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-slate-300">
            Empresa <span class="text-slate-500 font-normal">(opcional)</span>
          </label>
          <input
      ...
      <div v-if="tabActiva === 'seguridad'" class="bg-[#13151f] border border-white/5 rounded-2xl p-6 space-y-5">
        <div>
          ...
          <p class="text-xs text-slate-500 mt-1">Mínimo 8 caracteres, una mayúscula y un número</p>
        </div>

        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-slate-300">Contraseña actual</label>
          <div class="relative">
            <input
              v-model="formPass.contrasenaActual"
              :type="mostrarPassActual ? 'text' : 'password'"
              placeholder="Tu contraseña actual"
              class="w-full px-4 py-3 pr-11 rounded-xl bg-white/5 border text-white placeholder-slate-500 text-sm outline-none transition-colors"
              :class="erroresPass.contrasenaActual ? 'border-red-500/50' : 'border-white/10 focus:border-violet-500/50'"
            />
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white transition-colors"
              @click="mostrarPassActual = !mostrarPassActual"
            >
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-slate-300">Nueva contraseña</label>
          <div class="relative">
            <input
              ...
              class="w-full px-4 py-3 pr-11 rounded-xl bg-white/5 border text-white placeholder-slate-500 text-sm outline-none transition-colors"
              :class="erroresPass.contrasenaNueva ? 'border-red-500/50' : 'border-white/10 focus:border-violet-500/50'"
            />
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white transition-colors"
              @click="mostrarPassNueva = !mostrarPassNueva"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-slate-300">Confirmar nueva contraseña</label>
          <input
            v-model="formPass.confirmar"
            type="password"
            placeholder="Repite la nueva contraseña"
            class="w-full px-4 py-3 rounded-xl bg-white/5 border text-white placeholder-slate-500 text-sm outline-none transition-colors"
            :class="erroresPass.confirmar ? 'border-red-500/50' : 'border-white/10 focus:border-violet-500/50'"
          />
