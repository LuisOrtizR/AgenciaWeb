...
const OPCIONES_ORDEN: { valor: OrdenServicio; etiqueta: string }[] = [
      ...
      <div class="max-w-5xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8 flex-wrap gap-3">
          ...
          <div class="flex items-center gap-1 bg-white/4 border border-white/8 rounded-xl p-1">
            <button
              v-for="op in OPCIONES_ORDEN"
              :key="op.valor"
              class="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
              :class="orden === op.valor ? 'bg-violet-600 text-white shadow-sm' : 'text-gray-400 hover:text-white'"
        ...
        <div v-else-if="servicios.length === 0" class="py-20 flex flex-col items-center gap-4 text-center">
        ...
        <div v-else class="space-y-4">
          ...
          >
            ...
            <div class="lg:w-52 shrink-0 flex flex-col justify-between gap-4 lg:border-l lg:border-white/5 lg:pl-6">
              <div>
                <p class="text-xs text-gray-600 uppercase tracking-widest font-medium mb-1">Inversión desde</p>
                <p class="text-2xl font-black text-white leading-none">{{ cop(s.precioDesde) }}</p>
                <p v-if="s.precioDesdeUsd" class="text-sm text-gray-500 mt-1">≈ {{ usd(s.precioDesdeUsd) }} USD</p>
