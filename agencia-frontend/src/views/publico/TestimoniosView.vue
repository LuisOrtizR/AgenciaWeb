...
const setCalificacion = (n: number) => { formulario.calificacion = n }
const setHover        = (n: number) => { estrellaHover.value = n }
const clearHover      = ()          => { estrellaHover.value = 0 }
...
const enviar = async () => {
  ...
  try {
    ...
    const msg = e?.response?.data?.mensaje ?? 'Ocurrió un error al enviar. Intenta de nuevo.'
...
const colorAvatar = (nombre: string) =>
  coloresAvatar[nombre.charCodeAt(0) % coloresAvatar.length]
...
<template>
  <div class="bg-[#0a0a0f] min-h-screen">
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-20">
      ...
      <div class="mb-10">
        ...
        <div class="flex items-center gap-6 mt-4">
          <div class="flex items-center gap-2">
            <div class="flex">
              <span v-for="n in 5" :key="n" class="text-base" :class="n <= Math.round(promedioCalificacion()) ? 'text-yellow-400' : 'text-white/10'">★</span>
            </div>
          ...
          <div class="bg-[#111118] border border-white/8 rounded-2xl overflow-hidden">
            ...
            <div class="px-6 py-5 space-y-5">
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-400 uppercase tracking-wide">Calificación <span class="text-red-400">*</span></label>
                <div class="flex items-center gap-1" @mouseleave="clearHover">
                  ...
                  <span v-if="estrellaHover || formulario.calificacion" class="text-xs text-yellow-400 ml-2 font-medium">
                    {{ ['','Muy malo','Malo','Regular','Bueno','¡Excelente!'][estrellaHover || formulario.calificacion] }}
                  </span>
                </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-xs font-medium text-gray-400 uppercase tracking-wide">Nombre <span class="text-red-400">*</span></label>
                  <input
                ...
                <div class="space-y-1.5">
                  <label class="text-xs font-medium text-gray-400 uppercase tracking-wide">Empresa <span class="text-gray-600 normal-case">(opcional)</span></label>
                  <input
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-400 uppercase tracking-wide">Tu experiencia <span class="text-red-400">*</span></label>
                <textarea
                ...
                <div class="flex justify-between">
                  ...
                  <span class="text-xs text-gray-600 ml-auto" :class="formulario.texto.length > 900 ? 'text-yellow-400' : ''">
                    {{ formulario.texto.length }}/1000
                  </span>
                </div>
              </div>
      ...
      >
        <div v-if="enviado" class="mb-12 bg-emerald-500/8 border border-emerald-500/20 rounded-2xl px-6 py-5 flex items-center gap-4">
      ...
      <div v-else class="space-y-0 divide-y divide-white/4">
        <div
          v-for="t in testimonios"
          :key="t.id"
          class="py-5 flex gap-4 group"
        >
          <div
          ...
          <div class="flex-1 min-w-0">
            ...
            <div v-if="t.proyecto" class="mt-2 inline-flex items-center gap-1.5 text-xs text-violet-400 bg-violet-500/8 border border-violet-500/15 rounded-lg px-2.5 py-1">
