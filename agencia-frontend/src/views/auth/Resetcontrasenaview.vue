...
const etiquetaFortaleza = computed(() => ['', 'Débil', 'Regular', 'Buena', 'Fuerte'][fortaleza.value])
const colorFortaleza    = computed(() => ['', 'bg-red-500', 'bg-yellow-500', 'bg-blue-500', 'bg-green-500'][fortaleza.value])
...
const guardar = async () => {
  ...
  cargando.value = true
  try {
    await autenticacionServicio.resetContrasena({
      token:      token.value,
    ...
    const mensaje = error instanceof Error ? error.message : 'El enlace es inválido o ha expirado'
        ...
        <template v-if="tokenInvalido">
          <div class="text-center py-4 space-y-4">
            ...
            <RouterLink
              ...
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-violeta hover:bg-violeta/90 text-white text-sm font-semibold transition-all shadow-lg shadow-violeta/25"
            >
        ...
        <template v-else-if="exito">
          <div class="text-center py-4 space-y-4">
            ...
            <div>
              ...
              <p class="text-gris-medio text-sm mt-2">
