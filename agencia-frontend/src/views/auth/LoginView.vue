...
function extraerMensaje(err: unknown): string {
  if (err && typeof err === 'object' && 'response' in err) {
    ...
    return res?.data?.mensaje ?? 'Credenciales inválidas'
  }
...
const validar = (): boolean => {
  errores.value = {}
  if (!form.value.correo.trim())  errores.value.correo     = 'El correo es requerido'
  if (!form.value.contrasena)     errores.value.contrasena = 'La contraseña es requerida'
  ...
  <div class="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4 py-12">
    ...
    <div class="relative w-full max-w-md">
      ...
      <div class="bg-[#13151f] border border-white/8 rounded-2xl p-8 shadow-2xl shadow-black/40">
        <div class="space-y-5">
          ...
          <div class="space-y-1.5">
            <div class="flex items-center justify-between">
              ...
              <RouterLink :to="{ name: 'olvide-contrasena' }" class="text-xs text-violeta-claro hover:text-violeta transition-colors">
        ...
        <RouterLink :to="{ name: 'registro' }" class="block w-full py-3 rounded-xl border border-white/10 hover:border-white/20 text-gris-medio hover:text-white text-sm font-medium text-center transition-all">
          Crear cuenta gratis
        </RouterLink>
      </div>
